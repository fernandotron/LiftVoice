...
`;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-950/40 backdrop-blur-xs">
      <div className="relative w-full max-w-2xl max-h-[90vh] flex flex-col rounded-2xl border border-zinc-200 bg-white overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-150 text-zinc-900">
        ...
        <div className="p-4 sm:p-5 border-b border-zinc-200 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-zinc-500">
                Minuta estructurada de la conferencia en vivo (Sala: <span className="font-mono text-zinc-700 font-medium">{roomId}</span>)
              </p>
            </div>
          </div>
          ...
          <button
            onClick={onClose}
            className="p-1.5 rounded-lg border border-zinc-200 text-zinc-400 hover:text-zinc-800 hover:bg-zinc-100 transition-colors cursor-pointer"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="p-5 overflow-y-auto space-y-5 text-left">
          ...
          {isLoading && (
            <div className="py-14 text-center space-y-2.5">
              <Loader2 className="w-7 h-7 text-zinc-800 animate-spin mx-auto" />
          ...
          {error && !isLoading && (
            <div className="p-3.5 rounded-xl bg-red-50 border border-red-200 text-red-700 text-xs flex items-center gap-3">
              ...
              <div>
                <b className="block text-red-900 font-semibold mb-0.5">No se pudo generar el resumen</b>
                {error}
              </div>
            </div>
          )}
            ...
            <>
              ...
              <div className="grid grid-cols-3 gap-2.5">
                <div className="p-2.5 rounded-xl bg-zinc-50 border border-zinc-200 text-center">
                  <div className="text-[10px] text-zinc-400 uppercase font-mono">Duración</div>
                  <div className="text-sm font-semibold text-zinc-900 font-mono mt-0.5">
                    {duration} min
                  </div>
                </div>
                <div className="p-2.5 rounded-xl bg-zinc-50 border border-zinc-200 text-center">
                  <div className="text-[10px] text-zinc-400 uppercase font-mono">Frases</div>
                  <div className="text-sm font-semibold text-zinc-900 font-mono mt-0.5">
                    {totalSentences}
                  </div>
                </div>
                <div className="p-2.5 rounded-xl bg-zinc-50 border border-zinc-200 text-center">
                  <div className="text-[10px] text-zinc-400 uppercase font-mono">Oyentes</div>
                  <div className="text-sm font-semibold text-zinc-900 font-mono mt-0.5">
                    {totalListeners}
                  </div>
                </div>
              ...
              {keyPoints.length > 0 && (
                <div className="space-y-2">
                  <h5 className="text-xs font-semibold text-zinc-900 uppercase font-mono tracking-wider flex items-center gap-1.5">
        ...
        {/* Footer Actions */}
        {summaryData && !isLoading && (
          <div className="p-3.5 sm:p-4 border-t border-zinc-200 bg-zinc-50/50 flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
              <button
                ...
                className="h-8 px-3 rounded-lg border border-zinc-200 bg-white hover:bg-zinc-50 text-zinc-800 text-xs font-medium cursor-pointer shadow-2xs transition-colors flex items-center gap-1.5"
              >
                {copied ? (
                  <>
              ...
              <button
                ...
                className="h-8 px-3.5 rounded-lg bg-zinc-950 hover:bg-zinc-800 text-white text-xs font-medium cursor-pointer shadow-xs transition-colors flex items-center gap-1.5"
              >
