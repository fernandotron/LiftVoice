...
export default function HomeView({
  onCreateRoom = () => {},
  onJoinRoom = () => {},
  mode = 'attendee', // 'attendee' | 'creator'
  isAttendeeOnly = true,
  isCreatorOnly = false
}) {
  ...
  const isCreatorMode = mode === 'creator' || isCreatorOnly;
  ...
  return (
    <div className="flex-1 flex flex-col justify-between bg-white dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 transition-colors overflow-y-auto min-h-0">
      ...
      <div className="my-auto w-full max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 space-y-7 sm:space-y-9">
        ...
        {/* Píldora Minimalista de Reingreso a Última Sala (Solo en modo oyente) */}
        {!isCreatorMode && recentRoom && (
        ...
        {/* Action Card: Modo Creador (/create) vs Modo Oyente (/ y /join) */}
        <div className="space-y-4 sm:space-y-6 w-full max-w-md mx-auto">
          {isCreatorMode ? (
            /* Vista Exclusiva de Creación de Sala (Ruta /create o /host) */
            ...
            /* Vista Exclusiva de Oyente (Ruta principal / y /join) — SIN opción de crear sala */
            <div className="bg-white dark:bg-zinc-900/70 border border-zinc-200/90 dark:border-zinc-800 rounded-[28px] p-6 sm:p-7 flex flex-col justify-between shadow-2xs text-left space-y-6 w-full animate-fadeIn">
              <div className="space-y-4">
                <div className="w-12 h-12 rounded-2xl bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 flex items-center justify-center text-zinc-900 dark:text-zinc-100 shadow-2xs">
