...
import {
  Settings, Key, Check, Shield, Cpu, Zap, Volume2, Mic,
...
export default function AdminSettingsView({
  ...
  // Extract room ID from prop or search params if accessed via /admin?room=...
  ...
  const [activeTab, setActiveTab] = useState('stt'); // 'stt' | 'tts' | 'ai' | 'medical' | 'keys' | 'appearance'

  // STT Transcriber Engine (Default Deepgram Nova-3)
  const [sttEngine, setSttEngine] = useState(() => safeGetItem('lv_stt_engine', 'deepgram'));

  // TTS Global & Per-Language Voices (Default Deepgram Aura)
  ...
  // Audio cleanup on unmount or activeTab change
  useEffect(() => {
    return () => {
  ...
  // Load active server config on mount
  useEffect(() => {
    fetch('/api/config')
      .then(res => res.json())
      .then(data => {
  ...
  const handlePreviewVoice = async (langCode) => {
    ...
    try {
      ...
      if (e.name !== 'AbortError') {
        console.warn('[AdminSettingsView] Voice preview warning:', e);
      }
    } finally {
  ...
  return (
    <div className="min-h-dvh w-full max-w-full bg-white dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 flex flex-col justify-between transition-colors duration-150 font-sans">
        ...
        <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-15 sm:h-16">
          ...
          <div className="flex items-center gap-3 sm:gap-4 min-w-0">
            ...
            <div className="flex items-center gap-2 min-w-0">
              ...
              <span className="text-[10px] font-mono px-2 py-0.5 rounded-full bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 text-zinc-600 dark:text-zinc-400 font-semibold hidden sm:inline-block">
                Studio 2026
              </span>
          ...
          {/* Center / Room Badge (si existe sala activa) */}
          {effectiveRoomId && (
          ...
          {/* Right: Theme Switcher + Volver */}
          <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
            {/* Theme Toggle Button */}
            <button
              type="button"
              onClick={toggleTheme}
              className="w-9 h-9 rounded-full border border-zinc-200 dark:border-zinc-800 bg-zinc-100/80 dark:bg-zinc-900 hover:bg-zinc-200/80 dark:hover:bg-zinc-800 flex items-center justify-center text-zinc-700 dark:text-zinc-300 transition-colors cursor-pointer shadow-2xs"
            ...
            <button
              ...
              className="h-9 px-3.5 sm:px-4 rounded-full bg-zinc-900 dark:bg-white hover:bg-zinc-800 dark:hover:bg-zinc-200 text-white dark:text-zinc-950 font-semibold text-xs flex items-center gap-2 cursor-pointer shadow-2xs transition-all active:scale-95"
            >
              {effectiveRoomId ? (
                <>
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 flex flex-col md:flex-row gap-6 lg:gap-8">
        ...
        <aside className="w-full md:w-64 lg:w-72 flex-shrink-0 select-none">
          <div className="sticky top-24 space-y-4">
            ...
            {/* Tab Navigation List */}
            <div className="flex md:flex-col overflow-x-auto no-scrollbar gap-1.5 p-1.5 rounded-2xl bg-zinc-100/60 dark:bg-zinc-900/40 border border-zinc-200/80 dark:border-zinc-800/80">
              {TABS.map((tab) => {
                const Icon = tab.icon;
            ...
            {/* Quick Active Engine Status Box (Desktop only) */}
            <div className="hidden md:block p-4 rounded-2xl bg-zinc-50 dark:bg-zinc-900/50 border border-zinc-200/80 dark:border-zinc-800/80 space-y-2 text-zinc-600 dark:text-zinc-400 text-xs">
              <div className="flex items-center justify-between font-semibold text-zinc-900 dark:text-zinc-100">
                <span className="flex items-center gap-1.5">
                  <Zap className="w-3.5 h-3.5 text-amber-500" />
                  <span>Cadena Activa</span>
                </span>
                <span className="text-[9px] font-mono px-2 py-0.5 rounded-full bg-emerald-100 dark:bg-emerald-950/60 text-emerald-800 dark:text-emerald-300 font-bold border border-emerald-200 dark:border-emerald-800">
                  ONLINE
                </span>
              </div>
              <p className="text-[11px] leading-relaxed">
                STT: <strong className="text-zinc-900 dark:text-zinc-100">{sttEngine === 'deepgram' ? 'Deepgram Nova-3 ⚡' : sttEngine === 'webspeech' ? 'Web Speech API' : 'Whisper-1'}</strong>
              </p>
              <p className="text-[11px] leading-relaxed">
                Traducción: <strong className="text-zinc-900 dark:text-zinc-100">{preferredEngine === 'gemini' ? 'Gemini 3.1 Flash-Lite ⚡' : preferredEngine === 'qwen' ? 'Alibaba Qwen 3.8' : 'GPT-4o Mini'}</strong>
              </p>
              <p className="text-[11px] leading-relaxed">
                TTS: <strong className="text-zinc-900 dark:text-zinc-100">{preferredTtsEngine === 'deepgram' ? 'Deepgram Aura ⚡' : preferredTtsEngine === 'google' ? 'Google Neural Universal' : preferredTtsEngine}</strong>
              </p>
            </div>

            {/* Floating Save Action Bar (Desktop Sidebar) */}
            <div className="hidden md:block pt-1">
              <button
                type="button"
                onClick={handleSave}
                disabled={isSaving}
                className="w-full h-11 rounded-xl bg-zinc-950 dark:bg-white hover:bg-zinc-800 dark:hover:bg-zinc-200 text-white dark:text-zinc-950 text-xs font-semibold flex items-center justify-center gap-2 cursor-pointer shadow-sm transition-all active:scale-[0.99] disabled:opacity-50"
              >
                {isSaved ? (
                  <>
      ...
      {/* PIE DE PÁGINA                                                 */}
      {/* ───────────────────────────────────────────────────────────── */}
      <footer className="w-full border-t border-zinc-200/80 dark:border-zinc-800/80 bg-zinc-50/60 dark:bg-zinc-900/30 py-5 px-4 sm:px-8 transition-colors mt-auto flex-shrink-0">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-3 text-xs sm:text-sm">
          <div className="flex items-center gap-2 text-zinc-500 dark:text-zinc-400">
            <span className="font-semibold text-zinc-900 dark:text-zinc-100">LiftVoice</span>
            <span>&bull;</span>
            <span>Panel de Configuración y Motores de Traducción</span>
          </div>
          <div className="text-zinc-400 dark:text-zinc-500 font-mono text-xs">
            LiftVoice Engine &copy; 2026
          </div>
        </div>
      </footer>
